...
class GameObject(pygame.sprite.Sprite):
    ...
    def interact(self, console, score, logger, command=False):
...
class Cursor(pygame.sprite.Sprite):
    ...
    def interact(self, pen, game_objs, console, score, logger):
     ...
     #           for i in range(len(score)):
                if self.state == 'FREE':
                    ...
                    self.reward = [0, -2, +3, +4]
                    self.i_string = "You pat Pinja"
                elif self.state == 'THREATEN':
                    self.reward = [-2, +1, -3, -4]
                    self.i_string = "You threated Pinja"
                else:
                    self.reward = [0, 0, 0, 0]
...
class Penguin(pygame.sprite.Sprite):
    ...
    def move_to_state(self, new_state, game_objects, console, logger):
        if new_state != "act":
#            print(new_state)
...
#            print(vect_len, dist)
            ...
            if self.state in GameObject.states:
                for g in game_objects:
                    if g.state == self.state:
                        print(self.state)
                        self.score = g.interact(console, self.score, logger, self.commanded)
                        self.commanded = False
                        print(self.score)


    def move_with_probability(self, game_objects, console, new_state_code, logger):
        if self.next_state == None:
            ...
            self.move_to_state(self.arcs[self.state][new_state_code], game_objects, console, logger)
        else:
            self.move_to_state(self.next_state, game_objects, console, logger)
    '''
    ...
    def interact(self, game_objects, game_inventory, game_buttons, cur, console, logger):
        cursor = pygame.mouse.get_pos()
        x, y = cursor[0], cursor[1]
        ...
        self.score = cur.interact(self, game_objects, console, self.score, logger)
        if x <= 180:
...
class Maze(base.PyGameWrapper):
    ...
    #    self.game_scales = [Scale(st) for st in Scale.states]
        ...
        self.logger = Logger("exper_" + str(int(time())) + ".csv")
        self.cur = Cursor()
    ...
    def step(self, dt):
        ...
        if not self.is_game_over:
            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame.MOUSEBUTTONDOWN:
                    ...
                    if event.button == LEFT:
                        self.pen.interact(self.game_objects, self.game_inventory, self.game_buttons, self.cur, self.console, self.logger)
            ...
            self.pen.move_with_probability(self.game_objects, self.console, new_state_code, self.logger)
